...
'         False の場合は確認を省略します。
'         restoreProtection - True の場合は処理完了後に保護を復元します。
'                            False の場合は解除した状態を維持します。
'===============================================================================
Public Sub ClearMonthlyDataAndRefreshCalendar( _
    Optional ByVal showConfirm As Boolean = True, _
    Optional ByVal restoreProtection As Boolean = True)
...
CleanUp:
    ' --- 保護の復元 ---
    If wasProtected And restoreProtection Then
        On Error Resume Next
        If Len(pwd) > 0 Then
