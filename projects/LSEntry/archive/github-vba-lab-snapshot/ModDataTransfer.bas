...
' --- 動作設定・書式定数 ---
Private Const WORK_KEY_DELIMITER_CODE As Long = &H1F      ' WorkKeyエンコード時に使用する区切り文字 (Unit Separator)
Private Const WORK_KEY_ESCAPE_CODE As Long = &H1E         ' WorkKeyエンコード時に使用するエスケープ文字 (Record Separator)
...
' --- 列追加ポリシー設定（Enum/既定値は ModAppConfig.bas で定義）---
...
Private Type TransferConfig
    ...
    AddPolicy      As ColumnAddPolicy ' 新規列の追加ポリシー
End Type
...
'==============================================================================
' 【WorkKeyユーティリティ関数群】
' WorkKeyの生成・エンコード・デコード処理を提供します。
'==============================================================================

Private Function WorkKeyDelimiter() As String
    WorkKeyDelimiter = ChrW$(WORK_KEY_DELIMITER_CODE)
End Function

Private Function WorkKeyEscape() As String
    WorkKeyEscape = ChrW$(WORK_KEY_ESCAPE_CODE)
End Function

Private Function CreateWorkKey(ByVal category As String, ByVal workNo As String) As WorkKey
    Dim key As WorkKey
    key.Category = Trim$(category)
    key.WorkNo = Trim$(workNo)
    CreateWorkKey = key
End Function

Private Function WorkKeyToDictionaryKey(ByRef workKey As WorkKey) As String
    Dim delimiter As String

    delimiter = WorkKeyDelimiter()

    WorkKeyToDictionaryKey = EncodeWorkKeyComponent(workKey.Category) & _
                              delimiter & _
                              EncodeWorkKeyComponent(workKey.WorkNo)
End Function

Private Function DictionaryKeyToWorkKey(ByVal dictionaryKey As String) As WorkKey
    Dim key As WorkKey
    ...
    delimiter = WorkKeyDelimiter()
    escapeChar = WorkKeyEscape()

    For i = 1 To Len(dictionaryKey)
        ch = Mid$(dictionaryKey, i, 1)
        If ch = escapeChar Then
            i = i + 1
            If i <= Len(dictionaryKey) Then
                buffer = buffer & Mid$(dictionaryKey, i, 1)
            Else
                buffer = buffer & escapeChar
            End If
        ElseIf (ch = delimiter) And Not hasDelimiter Then
            key.Category = buffer
            buffer = vbNullString
            hasDelimiter = True
        Else
    ...
    key.WorkNo = Trim$(key.WorkNo)

    DictionaryKeyToWorkKey = key
End Function

Private Function EncodeWorkKeyComponent(ByVal value As String) As String
    ...
    result = Replace(value, escapeChar, escapeChar & escapeChar)
    result = Replace(result, delimiter, escapeChar & delimiter)
    EncodeWorkKeyComponent = result
End Function

Private Function FormatWorkKeyForDisplay(ByRef workKey As WorkKey) As String
    FormatWorkKeyForDisplay = "作業コード【" & workKey.Category & "】+作番【" & workKey.WorkNo & "】"
End Function
...
Private Function InitializeTransferConfig( _
    ...
    If config.targetRow = 0 Then
        ...
        If ret = vbYes Then
            Call ClearMonthlyDataAndRefreshCalendar(False, False)
    ...
    ' --- ステップ5：定数から動作設定を読み込み ---
    config.DryRun = ModAppConfig.DEFAULT_DRY_RUN_MODE
    config.AddPolicy = ModAppConfig.DEFAULT_COLUMN_ADD_POLICY
...
'           (Key=WorkKeyをエンコードした文字列, Value=合計分数)
'===============================================================================
Private Function AggregateTimeData(ByRef items As collection) As Object
    ...
    Dim workKey As WorkKey              ' 集計対象のキー
    Dim dictionaryKey As String         ' ディクショナリに保存するキー文字列
    ...
    For i = 1 To items.Count
        ...
        workKey = CreateWorkKey(CStr(v(1)), CStr(v(0)))
        dictionaryKey = WorkKeyToDictionaryKey(workKey)

        ' --- キーの存在に応じて、分数を加算または新規追加 ---
        If dic.Exists(dictionaryKey) Then
            dic(dictionaryKey) = dic(dictionaryKey) + CDbl(v(2)) ' 既存キー：加算
        Else
            dic.Add dictionaryKey, CDbl(v(2))                   ' 新規キー：追加
        End If
    Next
...
'===============================================================================
' 【機能名】列マッピングの構築
' 【概要】  「月次データ」シートのヘッダを解析し、WorkKeyと
'           列番号の対応表（WorkKeyをエンコードしたキーを持つディクショナリ）を作成します。
...
Private Sub BuildColumnMapping(ByRef wsMonthly As Worksheet, ByRef lastColOut As Long, ByRef mapDict As Object)
    ...
    Dim workKey As WorkKey              ' 列マッピング用のキー
    Dim dictionaryKey As String         ' ディクショナリ用のキー文字列
    ...
    For c = MonthlyCol_Min To lastCol
        ...
        If categoryName <> "" Then
            workKey = CreateWorkKey(categoryName, workNoName)
            dictionaryKey = WorkKeyToDictionaryKey(workKey)

            If Not mapDict.Exists(dictionaryKey) Then
                mapDict.Add dictionaryKey, c ' ディクショナリにキーと列番号を登録
            End If
        End If
    Next
End Sub
...
'           aggregatedData: WorkKeyをエンコードしたキーを持つ集計結果のディクショナリ
...
Private Function ShowPreviewAndConfirm(ByVal targetDate As Date, ByRef aggregatedData As Object) As Boolean
    ...
    Dim dictionaryKey As Variant        ' ディクショナリのキーを巡回するための変数
    Dim n As Long                       ' 表示件数カウンタ
    Dim workKey As WorkKey              ' 表示用のWorkKey
    ...
    ' --- 集計データの内容をメッセージに追加 ---
    For Each dictionaryKey In aggregatedData.Keys
        n = n + 1
        If n <= MAX_LINES Then
            ' --- 最大表示行数までは詳細を表示 ---
            workKey = DictionaryKeyToWorkKey(CStr(dictionaryKey))
            ' 作番、作業コード、時間をタブ区切りで追加
            msg = msg & workKey.WorkNo & PREVIEW_TAB & " | " & workKey.Category & PREVIEW_TAB & _
                  " | " & MinutesToHHMMString(aggregatedData(dictionaryKey)) & vbCrLf
        Else
...
'           wsMonthly: 「月次データ」シートオブジェクト
'           aggregatedData: WorkKeyをエンコードしたキーを持つ集計結果のディクショナリ
'           mapDict: WorkKeyをエンコードしたキーを持つ列マッピング情報
...
Private Sub WriteAggregatedDataToSheet( _
    ...
    ' --- 変数宣言 ---
    Dim dictionaryKey As Variant        ' ディクショナリのキー
    Dim targetCol As Long               ' 書き込み対象の列番号
    Dim workKey As WorkKey              ' 書き込み対象のWorkKey
    ...
    For Each dictionaryKey In aggregatedData.Keys
        ...
        targetCol = GetOrCreateColumn(workKey, config, wsMonthly, mapDict, lastCol, result)

        ' --- 有効な列が取得できた場合のみ書き込み実行 ---
        If targetCol > 0 Then
            ' ★★★ 変更点: 引数に config.targetDate を追加 ★★★
            WriteTimeDataToCell wsMonthly, config.targetRow, targetCol, aggregatedData(dictionaryKey), result, config.targetDate
...
'===============================================================================
' 【機能名】列の取得または新規作成
' 【概要】  指定されたWorkKey（作業コード＋作番）に対応する列番号を取得します。
'           存在しない場合は、設定されたポリシーに基づき新規作成を試みます。
' 【引数】  workKey: 作業コードと作番の組み合わせ
'           config: 転記処理の設定情報
'           wsMonthly: 「月次データ」シートオブジェクト
...
Private Function GetOrCreateColumn( _
    ...
    Dim newCol As Long
    Dim requiresConfirmation As Boolean

    dictionaryKey = WorkKeyToDictionaryKey(workKey)

    ' --- マッピングにキーが存在すれば、既存の列番号を返す ---
    If mapDict.Exists(dictionaryKey) Then
        GetOrCreateColumn = mapDict(dictionaryKey)
        Exit Function
    End If
    ...
    Select Case config.AddPolicy
        ...
        Case AddPolicy_Auto
            ...
            newCol = CreateNewColumn(workKey, wsMonthly, mapDict, lastCol)
    ...
    If requiresConfirmation Then
        If ConfirmColumnCreation(workKey) Then
            newCol = CreateNewColumn(workKey, wsMonthly, mapDict, lastCol)
            If newCol > 0 Then result.NewColumnsAdded = result.NewColumnsAdded + 1
            GetOrCreateColumn = newCol
        Else
...
Private Function CreateNewColumn( _
    ...
    ' --- ヘッダ情報（作業コードと作番）を設定 ---
    wsMonthly.Cells(MonthlyRow_Header, newCol).Value = workKey.Category
    wsMonthly.Cells(MonthlyRow_WorkNo, newCol).Value = workKey.WorkNo
    ...
    ' --- マッピング情報と最終列番号を更新 ---
    mapDict.Add WorkKeyToDictionaryKey(workKey), newCol
    lastCol = newCol
...
'===============================================================================
Private Function ConfirmColumnCreation(ByRef workKey As WorkKey) As Boolean
    ConfirmColumnCreation = (MsgBox( _
        FormatWorkKeyForDisplay(workKey) & "の列がありません。" & vbCrLf & _
...
'===============================================================================
' 【補助処理】最新データのクリップボードコピー
' 【概要】  「データ登録」シートから直近の有効な時間データを収集し、
'           タブ区切り形式でクリップボードにコピーします。
'===============================================================================
Public Sub CopyLatestDataToClipboard()
...
Private Function FindMatchingDateRow(ByRef wsMonthly As Worksheet, ByVal targetDate As Date) As Long
    ...
    Dim searchPatterns As Variant     ' 文字列検索用のパターン配列
    Dim pattern As Variant            ' パターン配列の要素を走査するための変数
    ...
    If foundCell Is Nothing Then
        searchPatterns = Array( _
            Format$(targetDate, "yyyy/mm/dd") & "*", _
            Format$(targetDate, "mm/dd") & "*", _
            Format$(targetDate, "m/d") & "*")
