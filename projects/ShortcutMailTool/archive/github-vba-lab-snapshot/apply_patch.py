...
module_path = Path(__file__).resolve().parent / 'ShortcutMailTool.bas'
module_path.write_text('''Attribute VB_Name = "ShortcutMailTool"
Option Explicit
...
#If VBA7 Then
    ...
    Private Declare PtrSafe Sub CopyMemory Lib "kernel32" Alias "RtlMoveMemory" (ByVal Destination As LongPtr, ByRef Source As Any, ByVal Length As LongPtr)
#Else
    ...
    Private Declare Sub CopyMemory Lib "kernel32" Alias "RtlMoveMemory" (ByVal Destination As Long, ByRef Source As Any, ByVal Length As Long)
#End If
...
Public Sub ShowShortcutMailMenu()
    ...
    MsgBox "コピーしました: " & items(selectedIndex)(0)
End Sub
...
#End If
    ...
    hMem = GlobalAlloc(GHND, dataSize)
    If hMem = 0 Then GoTo CleanFail

    pMem = GlobalLock(hMem)
    If pMem = 0 Then GoTo CleanFail

    CopyMemory ByVal pMem, data(LBound(data)), dataSize
    GlobalUnlock hMem
...
CleanFail:
    If hMem <> 0 And pMem <> 0 Then GlobalUnlock hMem
    GoTo CleanExit
End Function
''', encoding='utf-8')
